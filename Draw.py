...
#     # Undo: if left hand index and middle raised
#     if left_hand:
#         if left_hand.landmark[8].y < left_hand.landmark[6].y and left_hand.landmark[12].y < left_hand.landmark[10].y:
#             if draw_history:
#                 draw_history.pop()
#                 canvas[:] = 0
#                 for item in draw_history:
#                     if item[0] == 'line':
#                         cv2.line(canvas, *item[1:], item[3], item[4])
#                     elif item[0] == 'circle':
...
#         lx, ly = int(index_tip.x * w), int(index_tip.y * h)
#         check_click_with_left_hand(lx, ly)

#     # Right hand: drawing or manipulating
#     if right_hand:
...
#         # Brush size control with thumb-index distance
...
#     cv2.imshow("Two-Handed Gesture Drawing [With Undo, Size, Drag]", frame_display)
